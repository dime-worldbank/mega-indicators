...
def subnational_poverty_index():
    ...
    return (spark.table(f'indicator_intermediate.poverty_index_spid_gsap')
        .withColumn('region_name_tmp', regexp_replace(trim(regexp_replace(col("region_name"), "[-\[\]–]+", " ")), "[\\d]+", ""))
        .withColumn('region_name_alt', when(col("region_name_tmp").isin(['Maputo City', 'Maputo Cidade']), 'Cidade de Maputo').otherwise(col("region_name_tmp")))
