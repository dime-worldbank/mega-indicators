...
ddf = df[(df['Country Code'].map(lambda x: x[:3]=='TUN'))&(df['Indicator Code']=='SP.POP.TOTL')]
ddf = ddf.copy()
...
sdf.write.mode("overwrite").saveAsTable(f"{database_name}.tun_subnational_population")
