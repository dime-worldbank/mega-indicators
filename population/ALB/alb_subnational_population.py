...
ddf = df[(df['Country Code'].map(lambda x: x[:3]=='ALB'))&(df['Indicator Code']=='SP.POP.TOTL')]
ddf = ddf.copy()
...
assert ddf_pop.shape[0] >= 204, f'Expect at least 204 rows, got {ddf_pop.shape[0]}'
...
assert ddf_pop.adm1_name.nunique() == 12, f'Expected 12 counties, got {ddf_pop.adm1_name.nunique()}'
