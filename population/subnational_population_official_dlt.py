...
country_codes = ['moz', 'pry', 'ken', 'pak', 'bfa', 'col', 'cod', 'tun', 'btn', 'chl', 'nga', 'bgd', 'alb', "zaf", 'chl', 'gha', 'lbr', 'tgo', 'bdi']

@dlt.table(name=f'subnational_population')
def subnational_population():
    # Consolidating all the country specific dataframes
    dfs = [spark.table(f'{code}_subnational_population_silver') for code in country_codes]
    result_df = dfs[0]
    for df in dfs[1:]:
